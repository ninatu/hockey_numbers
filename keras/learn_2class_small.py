...
lr_reducer = ReduceLROnPlateau(monitor='val_loss', factor=0.1, cooldown=0, patience=5, verbose=1, min_lr=1e-7)
early_stopper = EarlyStopping(monitor='val_acc', min_delta=0.001, patience=10)
csv_logger = CSVLogger('./model/ftvgg16_2class_small_log.csv', append=True)
checkpoint = ModelCheckpoint('./model/checkpoints/ftvgg16_2class_small/weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto')
...
predictions = Dense(nb_outputs, activation='sigmoid', name='sigmoid')(x)
...
model.save("model/ftvgg16_2class_small.h5" )
