...
class NumberRegions(object):
    #area, x, y, w, h, centr_x, centr_y, perimeter. All normalized
    minParam = [-0.04,  -0.06,  -0.039,  0.189, -0.002,  0.164,  0.14,   0.147, -1]
    maxParam = [ 0.271,  0.502,  0.31,   0.813,  0.661,  0.783,  0.455,  1.389, 40]

    @staticmethod
    def filter_for_numbers(seg, regions):
        filt = np.zeros(len(regions['label'])).astype('bool')
        for idx,l in enumerate(regions['label']):
            mask = np.array(seg == l, dtype=np.uint8)
            params = NumberRegions._getParams(mask)
            if np.all(params > NumberRegions.minParam) and np.all(params < NumberRegions.maxParam):
                filt[idx] = True
        return filt
    ...
    def _getParams(mask):
        pars = []
        pars.append(NumberRegions._getArea(mask))
        pars.extend(NumberRegions._getRect(mask))
        pars.extend(NumberRegions._getCentroid(mask))
        pars.append(NumberRegions._getPerimetr(mask))
        pars.append(NumberRegions._getBoardIntersect(mask))
        return np.array(pars)
    
    @staticmethod 
    def _getCentroid(mask):
    ...
    @staticmethod
    def _getPerimetr(mask):
        mask = mask.copy()
        contour = cv2.findContours(mask, mode=cv2.RETR_LIST , method=cv2.CHAIN_APPROX_NONE)
        perimeter = contour[0][0].shape[0] #np.array(contour[0] > 1, dtype=np.uint8).sum()
        h, w = mask.shape
        normPer = perimeter / (h + w)
        return normPer

    @staticmethod
    def _getBoardIntersect(mask):
        h = mask.shape[0]
        w = mask.shape[1]
        board = np.ones(mask.shape, dtype=mask.dtype)
        board[1:h - 1, 1:w - 1] = 0
        return np.logical_and(mask, board).sum()
...
class TextRegions(object):
    ...
    minWidth = 25#30 #px
    minHeight = 25#30 #px
    minAspect = 0.05#0.3 # w > 0.3*h
    maxAspect = 20#7
    minArea = 50#100 # number of pix
    pArea = 0.01#0.60 # area_obj/area_minrect >= 0.6

    # RANSAC planar fitting params:
    dist_thresh = 0.1 #0.10 # m
    num_inlier = 2
...
class RendererV3(object):
    ...
    def __init__(self, data_dir, max_time=None):
        ...
        self.min_char_height = 5 #8 #px
        self.min_asp_ratio = 0.2 #0.4 #
    ...
    def filter_for_numbers(self, seg, regions):
        filt = NumberRegions.filter_for_numbers(seg, regions)
        regions = self.filter_regions(regions, filt)
        return regions
    ...
    def render_text(self,rgb,depth,seg,area,label,ninstance=1,viz=False):
        ...
        try:
            ...
            #DEBUG
            regions = self.filter_for_numbers(seg, regions)
        ...
        for i in xrange(ninstance):
            ...
            NUM_REP = 1 # re-use each region three times: MY CHANGE
